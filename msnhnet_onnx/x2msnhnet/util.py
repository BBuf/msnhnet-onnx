...
from msnhnet_onnx.x2oneflow.onnx2flow import from_onnx, from_pytorch, from_paddle, from_tensorflow2

def msnhnet_code_gen_func(input_size, model_weight_save_dir):
    ...
    for x in msnhnet_code_gen:
        f.write('     {}'.format(x))
    
    res = msnhnet_code_gen[len(msnhnet_code_gen)-1].split()[0]
...
def load_pytorch_module_and_check(
    ...
    if msnhnet_code_gen_flag == True and len(input_size) == 4:
        msnhnet_code_gen_func(input_size, model_weight_save_dir)
